...
# 라우터 정보 기입
router = APIRouter(
    prefix="/recommend/lecture/coldstart",
...
async def recommend_lecture_cold_start(request: Request, target_user_id: int):
    ...
    similar_user_lecture = user_lecture[user_lecture.user_id.isin(similar_user_id)]
    # print(similar_user_lecture.head())


    # 비슷한 유저의 lecture rating (찜한 강의는 + 10점, 취미와 같으면 + 5점)
    # user_hobby_rating 기록하기 위한 dataframe 만들기
    user_lecture_rating = pd.DataFrame(similar_user_lecture, columns = ['user_id', 'lecture_id', 'rating'])
    # print(user_lecture_rating.head())

    # 비슷한 유저가 선택한 강의 id들
    similar_user_lecture_id = list(set(user_lecture_rating.lecture_id.values.tolist()))
    # print(similar_user_lecture_id)

    # 타겟 유저의 취미에 해당하는 강의 id
    target_lecture = lecture_list[lecture_list["category_name"].isin(target_hobby)]
    # print(target_lecture.head())
    target_lecture_id = target_lecture["lecture_id"].values.tolist()
